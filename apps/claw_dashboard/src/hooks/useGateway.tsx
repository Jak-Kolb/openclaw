...
export const GatewayProvider = ({ children }: GatewayProviderProps) => {
  ...
  const checkStatus = async () => {
    ...
    try {
      const result = await gatewayClient.gatewayStatus();
      setStatus({ 
  ...
  const startGateway = async () => {
    setStatus(prev => ({ ...prev, loading: true }));
    try {
      const result = await gatewayClient.gatewayStart();
      if (result.success) {
  ...
  const stopGateway = async () => {
    setStatus(prev => ({ ...prev, loading: true }));
    try {
      const result = await gatewayClient.gatewayStop();
      if (result.success) {
        // Wait a moment then check status
        setTimeout(checkStatus, 1000);
      } else {
  ...
  const restartGateway = async () => {
    setStatus(prev => ({ ...prev, loading: true }));
    try {
      const result = await gatewayClient.gatewayRestart();
      if (result.success) {
        // Wait a moment then check status
        setTimeout(checkStatus, 1000);
      } else {
  ...
  const fetchLogs = async (lines: number = 50) => {
    try {
      const result = await gatewayClient.gatewayLogs(lines);
      if (result.logs) {
        setLogs(result.logs);
